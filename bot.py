...
async def giveaway_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_ID:
        return
    await update.message.reply_text("Отправьте фото для конкурса или /skip")
    return PHOTO
...
async def join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("SELECT * FROM giveaway_participants WHERE giveaway_id=? AND user_id=?", (giveaway_id, user.id))
    if cursor.fetchone():
        return

    cursor.execute("INSERT INTO giveaway_participants (giveaway_id,user_id,username) VALUES (?,?,?)",
...
async def reroll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("SELECT id, winners_count FROM giveaways ORDER BY id DESC LIMIT 1")
    g = cursor.fetchone()
    if not g:
        return
    giveaway_id, winners_count = g
    ...
    winners = participants[:winners_count]
    winners_text = "\n".join([f"@{w[0]}" for w in winners])
    await query.message.reply_text(f"🔄 <b>Новые победители:</b>\n{winners_text}", parse_mode="HTML")
